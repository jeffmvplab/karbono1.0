...
export const alertFactorDePrecipitacion = (prescription: IPrescriptions) => {
  const concDeProteinas: number =
    getConcentracionDeProteinas(prescription).volumen;
  ...
  } else if (1 < concDeProteinas && concDeProteinas < 1.5) {
    if (factor < 2) {
      resp.alert = "SEGURA";
    } else {
      resp.alert = "REVISAR";
...
export const alarmConcDeProteinas = (prescription: IPrescriptions) => {
  ...
  let concDeProteinas: number =
    ((aminoacidos * conAminoacidos + dipeptiven * 0.2) / volTotalNPT) * 100;
  const resp: IAlarm = { value: 0, alert: "" };
...
export const alarmConcSodio = (prescription: IPrescriptions) => {
  ...
  let sodioVol: number =
    (sodio * 2 + aporte_sodio_del_fosfato! * prescription?.peso!) /
    (volTotalNPT / 1000);
