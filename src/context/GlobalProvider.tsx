...
export const GlobalProvider: FC<Props> = ({ children }) => {
  ...
  const [messageErrorPasswordConfirm, setMessageErrorPasswordConfirm] =
    React.useState("");
  ...
  const [messageErrorNameYApellidos, setMessageErrorNameYApellidos] =
    React.useState("");
  ...
  const [messageErrorRegistroMedico, setMessageErrorRegistroMedico] =
    React.useState("");
  ...
  const [messageErrorEntidadDeSalud, setMessageErrorEntidadDeSalud] =
    React.useState("");
  ...
  const [errorCentralDeMezclas, setErrorCentralDeMezclas] =
    React.useState(false);
  const [messageErrorCentralDeMezclas, setMessageErrorCentralDeMezclas] =
    React.useState("");

  const handleCentralDeMezclas = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
  ...
  const [politica_de_privacidad, setPolitica_de_privacidad] =
  ...
  const [errorCodigoVerificacion, setErrorCodigoVerificacion] =
    React.useState(false);
  const [messageErrorCodigoVerificacion, setMessageErrorCodigoVerificacion] =
    React.useState("");
  ...
  const authStatus = (rol?: string) => {
    ...
    if (localStorageProtocol.get(StorageKeysEnum.user) !== null) {
      ...
      if (rol === "Administrador") {
        router.push(mainRoutes.loginEmpleados);
      } else {
        router.push(mainRoutes.loginCliente);
      }
      // console.log('IsAuth:', isAuth)
    }
  };
  ...
  const login = async () => {
    setLoadingAuth(true);
    // console.log('Loading...')
    const resp = await useruseCase.login(
      email,
      password,
      [entidadDeSalud],
      centralDeMezclas
    ...
    if (resp.statusCode === 201) {
      ...
      Cookies.set(CookiesKeysEnum.token, resp.body.token, {
        sameSite: "Strict",
      });
      Cookies.set(CookiesKeysEnum.userName, resp.body.nombre_apellidos, {
        sameSite: "Strict",
      });
    }
  ...
  const invitarUsuarios = async () => {
    ...
    const resp = await useruseCase.invitarUsuarios(
      userInv?.roles!,
      userInv?.email!,
      userInv?.nombre_apellidos!
    );
  ...
  const registerByInvitation = async () => {
    ...
    const resp = await useruseCase.registerByInvitation(
      ...
      phone!,
      password,
      userInv.roles[0] === RolUsersKeysEnum.prescriptor
        ? registroMedico!
        : undefined,
      name!,
      apellido!,
  ...
  const handleOpenModalRecoveryPass = () => {
    setOpenModalRecoveryPass(true);
  };
  ...
  const handleOpenModalVerifyPass = () => {
    setOpenModalVerifyPass(true);
  };
  ...
  const verificarCodigoRecoveryPassword = async () => {
    ...
    const resp = await useruseCase.verificarCodigoRecuperacion(
      email,
      codigoVerificacion,
      password
    );
    ...
    if (resp.statusCode === 201) {
      ...
      Cookies.set(CookiesKeysEnum.userRol, resp.body.rol[0], {
        sameSite: "Strict",
      });
      // }
  ...
  const [messageErroruserInvitado, setMessageErroruserInvitado] =
    React.useState("");
  ...
  return (
    <GlobalContext.Provider
      value={{
        ...
        setuserInvitado,
        erroruserInvitado,
        setErroruserInvitado,
        messageErroruserInvitado,
        setMessageErroruserInvitado,
        aceptarInvitacion,
